...
parser.add_argument('-ce',
                    '--corruption_eval',
                    action ='store_true',
                    help='convert only val corrupted'
                    )
...
def iterate(mode, args, loader, model, optimizer, logger, epoch):
    ...
    if args.corruption_eval:
        if mode == "test_completion":
                ruta = Path(args.detpath)
                with open(os.path.join(ruta.parent, 'ImageSets/val.txt'), 'r') as f:
    ...
    for i, batch_data in enumerate(loader):
        ...
        if mode == "test_completion":
            ...
            if args.corruption_eval:
                vis_utils.save_depth_as_points(pred, ids[i], args.detpath)
            else:
