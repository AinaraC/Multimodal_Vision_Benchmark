...
def clean_data(gt_anno, dt_anno, current_class, difficulty, eval_params):
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    MAX_TRUNCATION = [0.15, 0.3, 0.5]
    MIN_DISTANCE = eval_params["min_distance"]
    MAX_DISTANCE = eval_params["max_distance"]
    ...
    for i in range(num_gt):
        ...
        distance = gt_anno["location"][i][2]                                    # TODO: z-value or eucledian distance?
        ...
        if ((gt_anno["occluded"][i] > MAX_OCCLUSION[difficulty])
                ...
                or (height <= MIN_HEIGHT[difficulty]) or (distance < MIN_DISTANCE) or (distance > MAX_DISTANCE)):
    ...
    for i in range(num_dt):
        ...
        distance = dt_anno["location"][i][2] 

        if (height < MIN_HEIGHT[difficulty]) or (distance < MIN_DISTANCE) or (distance > MAX_DISTANCE):
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
        else:
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, eval_params):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty, eval_params)
...
def eval_class(gt_annos,
    ...
    Args:
        ...
        eval_params:        dict, e.g. used for distance based evaluation
        num_parts: int. a parameter for fast calculate algorithm
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, eval_params)
...
def do_eval(gt_annos,
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 0,
                     min_overlaps, eval_params, compute_aos)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    distance_thresholds = [(0, 80),
                           (0, 30), (30, 50), (50, 80)]
    ret_dict = {}
    df = pd.DataFrame(columns=['class', 'metric', 'difficulty', 'min dist', 'max dist', 'result'])

    for distance_values in distance_thresholds:
        min_dist, max_dist = distance_values

        eval_params = {'min_distance': min_dist,
                        'max_distance': max_dist}
        
        mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval(
            gt_annos, dt_annos, current_classes, min_overlaps, eval_params, compute_aos, PR_detail_dict=PR_detail_dict)
        ...
        for j, curcls in enumerate(current_classes):
            ...
            result += print_str(f'\n###############################')
            result += print_str(f'{class_to_name[curcls]} ({min_dist}-{max_dist}m)')
            result += print_str(f'###############################')
            ...
            for i in range(min_overlaps.shape[0]):
                ...
                if compute_aos:
                    ...
                    if (i == 0): 
                        ...
                        df.loc[len(df)] = [class_to_name[curcls], 'aos', 'easy', min_dist, max_dist, mAPaos_R40[j, 0, 0]]
                        df.loc[len(df)] = [class_to_name[curcls], 'aos', 'moderate', min_dist, max_dist, mAPaos_R40[j, 1, 0] ]
                        df.loc[len(df)] = [class_to_name[curcls], 'aos', 'hard', min_dist, max_dist, mAPaos_R40[j, 2, 0] ]
                ...
                if (i == 0) :
                    ...
                    df.loc[len(df)] = [class_to_name[curcls], '3d', 'easy', min_dist, max_dist, mAP3d_R40[j, 0, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], '3d', 'moderate', min_dist, max_dist, mAP3d_R40[j, 1, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], '3d', 'hard', min_dist, max_dist, mAP3d_R40[j, 2, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], 'bev', 'easy', min_dist, max_dist, mAPbev_R40[j, 0, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], 'bev', 'moderate', min_dist, max_dist, mAPbev_R40[j, 1, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], 'bev', 'hard', min_dist, max_dist, mAPbev_R40[j, 2, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], 'image', 'easy', min_dist, max_dist, mAPbbox_R40[j, 0, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], 'image', 'moderate', min_dist, max_dist, mAPbbox_R40[j, 1, 0]]
                    df.loc[len(df)] = [class_to_name[curcls], 'image', 'hard', min_dist, max_dist, mAPbbox_R40[j, 2, 0]]
    
    return result, ret_dict, df
