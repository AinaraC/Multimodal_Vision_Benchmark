...
class KittiMetric(BaseMetric):
    ...
    def __init__(self,
                 ann_file: str,
                 corruption_savepath: str = None ,
                 model_name: str = None,
                 corruption_eval: bool = False ,
        ...
        self.corruption_eval = corruption_eval
        self.corruption_savepath = corruption_savepath
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ap_dict, result_df = self.kitti_evaluate(
                result_dict,
                gt_annos,
                metric=metric,
                logger=logger,
        ...
        if self.corruption_eval:
            data_root = self.ann_file.split('/')[1]
            if data_root != 'kitti':
                result_df['corruption'] = self.ann_file.split('/')[2]
                result_df['severity'] = self.ann_file.split('/')[3].split('_')[-1]
            else:
                result_df['corruption'] = 'original'
                result_df['severity'] = '0'
            
            file_path = path.join(self.corruption_savepath,f'{self.model_name}_corruption_eval.csv')
            ok = path.isfile(file_path)
    ...
    def kitti_evaluate(self,
        ...
        for name in results_dict:
            ...
            ap_result_str, ap_dict_, df = kitti_eval(
