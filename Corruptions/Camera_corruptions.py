...
class ImageAddDarkness:
    ...
    def __call__(self, image) -> np.array:
        ...
        s = [0.60, 0.50, 0.40, 0.30, 0.2][severity-1]
        image = np.array(image) / 255.
        image_scaled = self.imadjust(image, image.min(), image.max(), 0, s, gamma=2.) * 255
        image_scaled = self.poisson_gaussian_noise(image_scaled, severity=severity)
        return image_scaled
    ...
    def poisson_gaussian_noise(self, image, severity):
        s_poisson = 10 * [45, 35 ,25, 12, 6][severity-1]
        image = np.array(image) / 255.
        image = np.clip(np.random.poisson(image * s_poisson) / s_poisson, 0, 1) * 255
        c_gauss = 0.1 * [.08,  0.18, 0.26, 0.34, 0.42][severity-1]
        image = np.array(image) / 255.
        image = np.clip(image + np.random.normal(size=image.shape, scale= c_gauss), 0, 1) * 255
        return np.uint8(image)
        

class ImageAddBrightness:
    def __init__(self, severity, seed) -> None:
        self.severity = severity
        np.random.seed(seed)    
    def __call__(self, image) -> np.array:
        """
            ...
            in uint8 (0~255) and RGB            points should be tensor : N * 4        
        """
        severity = self.severity       
        s = [0.3, 0.4, 0.5, 0.6, 0.7][severity - 1]       
        image_rgb_255 = image
        image_hsv = cv2.cvtColor(image_rgb_255, cv2.COLOR_RGB2HSV).astype(np.int64)
        image_hsv[:, :, 2] = np.clip(image_hsv[:, :, 2] + 256 * s, 0, 255)
        image_hsv = image_hsv.astype(np.uint8)
        image_aug = cv2.cvtColor(image_hsv, cv2.COLOR_HSV2RGB)
        return image_aug 
...
#         """
#             image should be numpy array : H * W * 3
#             in uint8 (0~255) and RGB            points should be tensor :       
#         """
#         severity = self.severity       
#         s = [0.3, 0.4, 0.5, 0.6, 0.7][severity - 1]    
#         x = np.array(x) / 255.   
...
class ImageAddSunlight:
    def __init__(self, severity, seed):
        self.intensity = severity
        np.random.seed(seed)
    ...
    def __call__(self, image):
        ...
        """
        severity = self.intensity
        s = [.5, .6, .7, .8, .9][severity - 1]
        ...
        y_coords, x_coords = np.meshgrid(np.arange(h), np.arange(w), indexing='ij')
      	...
      	# Generate projection
        direction_x = np.sin(np.radians(angle))
        direction_y = np.cos(np.radians(angle))
        projection = y_coords * direction_y + x_coords * direction_x

        # Normalize
        min_proj = projection.min()
        max_proj = projection.max()
        norm = (projection - min_proj) / (max_proj - min_proj)

        # Generate gradient
        gradient = s * ((1 - norm) * (1 - 0.4) + 0.4)

        # Paso 5: Convertir a RGB y escalar
        sunlight_rgb = np.repeat((gradient[:, :, np.newaxis] * 255), 3, axis=2).astype(np.uint8)

        # Paso 6: Mezclar con la imagen original
        image_aug = cv2.addWeighted(image, 1, sunlight_rgb, 1, 0)
